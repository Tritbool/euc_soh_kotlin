...
class SohTrendCusumChartGenerator(private val context: Context) {
    ...
    companion object {
        ...
        const val COLOR_BLACK = 0x00000000.toInt()
    ...
    fun generateAllTrendCharts(
        ...
        metricsWithExtractors(TREND_METRICS).mapNotNull { (metric, extractor) ->
            try {
                metric.csv_code to generateTrendChart(stats, metric, extractor, wheelName)
            } catch (_: Exception) {
                null
            }
        }
    ...
    fun generateCusumChart(
        ...
        if (alarmEntries.isNotEmpty())
            datasets.add(LineDataSet(alarmEntries, "Change detected (CUSUM)").apply {
                color = COLOR_ALARM_RED; setCircleColor(COLOR_ALARM_RED); setCircleHoleColor(COLOR_BLACK);
                lineWidth = 2f; circleRadius = 5f; setDrawValues(false)
                mode = LineDataSet.Mode.LINEAR
            })

        chart.data =
            LineData(datasets as List<com.github.mikephil.charting.interfaces.datasets.ILineDataSet>)
    ...
    fun generateAllCusumCharts(
        ...
        metricsWithExtractors(CUSUM_METRICS).mapNotNull { (metric, extractor) ->
            try {
                metric.csv_code to generateCusumChart(stats, metric, extractor, wheelName)
            } catch (_: Exception) {
                null
            }
        }
    ...
    fun generateInflexionChart(
        ...
        for (i in xVals.indices) {
            ...
            val windowY = xVals.zip(yVals).filter { (x, _) -> abs(x - kmI) <= halfW }
                .map { (_, y) -> y.toDouble() }
        ...
        yAxis.addLimitLine(
            LimitLine(
                limit.toFloat(),
                "danger threshold ≈ ${String.format("%.2f", limit)}"
            ).apply {
    ...
    fun generateAllInflexionCharts(
        ...
        metricsWithExtractors(TREND_METRICS).mapNotNull { (metric, extractor) ->
            try {
                metric.csv_code to generateInflexionChart(stats, metric, extractor, wheelName)
            } catch (_: Exception) {
                null
            }
        }
     ...
     */
    private fun metricsWithExtractors(danger_metrics: Set<Metrics>? = null): List<Pair<Metrics, (ReqStatsResult) -> Double?>> =
        if (danger_metrics != null) {
            danger_metrics.mapNotNull { metric ->
